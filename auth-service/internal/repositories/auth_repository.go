...
func (r *AuthRepository) CheckUserExists(ctx context.Context, emailOrPhone string) (bool, error) {
	var exists bool
	err := r.db.GetContext(ctx, &exists, "SELECT EXISTS(SELECT 1 FROM users WHERE email = $1 OR phone = $2)", emailOrPhone, emailOrPhone)
	return exists, err
}
...
func (r *AuthRepository) CreateAuthCode(ctx context.Context, code *models.AuthCode) error {
	err := r.db.QueryRowxContext(ctx, `
		INSERT INTO auth_codes (user_id, code, channel, expires_at, recipient)
...
func (r *AuthRepository) VerifyAuthCode(ctx context.Context, recipient string, code string) (bool, error) {
	var isValid bool
	err := r.db.GetContext(ctx, &isValid, "SELECT EXISTS(SELECT 1 FROM auth_codes WHERE code = $1 AND used = false AND recipient = $2)", code, recipient)
	return isValid, err
}
...
func (r *AuthRepository) GetAuthCodeByCodeAndUserID(ctx context.Context, code string, userID string) (*models.AuthCode, error) {
	...
	err := r.db.GetContext(ctx, &authCode, "SELECT * FROM auth_codes WHERE code = $1 AND user_id = $2", code, userID)
	return &authCode, err
}

func (r *AuthRepository) GetAuthCodeByID(ctx context.Context, codeID string) (*models.AuthCode, error) {
	var authCode models.AuthCode
	err := r.db.GetContext(ctx, &authCode, "SELECT * FROM auth_codes WHERE code_id = $1", codeID)
	return &authCode, err
}

func (r *AuthRepository) DeleteAuthCodeByID(ctx context.Context, codeID string) error {
	_, err := r.db.ExecContext(ctx, "DELETE FROM auth_codes WHERE code_id = $1", codeID)
	return err
}
